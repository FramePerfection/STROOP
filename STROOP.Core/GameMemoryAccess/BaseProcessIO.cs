...
public abstract class BaseProcessIO : IEmuRamIO
{
    ...
    {
        string? result = messageLogBuilder.ToString();
        messageLogBuilder.Clear();
        return result;
    }

    public bool ReadRelative(uint address, byte[] buffer, EndiannessType endianness) => ReadAbsolute(GetAbsoluteAddress(address, buffer.Length), buffer, endianness);

    private static readonly byte[] _swapByteOrder = new byte[] { 0x03, 0x02, 0x01, 0x00 };
    ...
    public bool WriteRelative(uint address, byte[] buffer, EndiannessType endianness) => WriteAbsolute(GetAbsoluteAddress(address, buffer.Length), buffer, endianness);
