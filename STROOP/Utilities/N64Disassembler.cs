...
namespace STROOP.Utilities
{
    ...
    public static class N64Disassembler
    {
        ...
        public static uint _fd(uint i)
        {
            return (i >> 6) & 0x1f;
        }

        public static uint _fs(uint i)
        {
            return (i >> 11) & 0x1f;
        }

        public static uint _ft(uint i)
        {
            return (i >> 16) & 0x1f;
        }

        public static uint _copop(uint i)
        {
            return (i >> 21) & 0x1f;
        }

        public static uint _offset(uint i)
        {
            return (i) & 0xffff;
        }

        public static uint _sa(uint i)
        {
            return (i >> 6) & 0x1f;
        }

        public static uint _rd(uint i)
        {
            return (i >> 11) & 0x1f;
        }

        public static uint _rt(uint i)
        {
            return (i >> 16) & 0x1f;
        }

        public static uint _rs(uint i)
        {
            return (i >> 21) & 0x1f;
        }

        public static uint _op(uint i)
        {
            return (i >> 26) & 0x3f;
        }

        public static uint _tlbop(uint i)
        {
            return i & 0x3f;
        }

        public static uint _cop1_func(uint i)
        {
            return i & 0x3f;
        }

        public static uint _cop1_bc(uint i)
        {
            return (i >> 16) & 0x3;
        }

        public static uint _target(uint i)
        {
            return (i) & 0x3ffffff;
        }

        public static uint _imm(uint i)
        {
            return (i) & 0xffff;
        }

        public static short _imms(uint i)
        {
            return (short)_imm(i);
        }   // treat immediate value as signed

        public static uint _base(uint i)
        {
            return (i >> 21) & 0x1f;
        }


        private static uint _branchAddress(uint a, uint i)
        {
            return (uint)((a + 4) + _imms(i) * 4);
        }

        private static uint _jumpAddress(uint a, uint i)
        {
            return (a & 0xf0000000) | (_target(i) * 4);
        }
        ...
        class Instruction
        {
            ...
            public string rt_d()
            {
                var reg = gprRegisterNames[_rt(this.opcode)];
                return makeRegSpan(reg);
            }

            public string rd()
            {
                var reg = gprRegisterNames[_rd(this.opcode)];
                return makeRegSpan(reg);
            }
            ...
            public string rs()
            {
                var reg = gprRegisterNames[_rs(this.opcode)];
                return makeRegSpan(reg);
            }

            // dummy operand - just marks ra as being a dest reg
            public string writesRA()
            {
                return "";
            }

            // cop1 regs
            public string ft_d(string fmt)
            {
                var reg = getCop1RegisterName(_ft(this.opcode), fmt);
                return makeFPRegSpan(reg);
            }

            public string fs_d(string fmt)
            {
                var reg = getCop1RegisterName(_fs(this.opcode), fmt);
                return makeFPRegSpan(reg);
            }

            public string fd(string fmt)
            {
                var reg = getCop1RegisterName(_fd(this.opcode), fmt);
                return makeFPRegSpan(reg);
            }

            public string ft(string fmt)
            {
                var reg = getCop1RegisterName(_ft(this.opcode), fmt);
                return makeFPRegSpan(reg);
            }

            public string fs(string fmt)
            {
                var reg = getCop1RegisterName(_fs(this.opcode), fmt);
                return makeFPRegSpan(reg);
            }
            ...
            public string gt_d()
            {
                var reg = cop2RegisterNames[_rt(this.opcode)];
                return makeRegSpan(reg);
            }

            public string gd()
            {
                var reg = cop2RegisterNames[_rd(this.opcode)];
                return makeRegSpan(reg);
            }
            ...
            public string gs()
            {
                var reg = cop2RegisterNames[_rs(this.opcode)];
                return makeRegSpan(reg);
            }

            public string imm()
            {
                return toHex(_imm(this.opcode), 16);
            }

            public string immwd()
            {
                return toHex(_imm(this.opcode), 16) + " (" + _imms(this.opcode) + ")";
            }

            public string branchAddress()
            {
                this.target = _branchAddress(this.address, this.opcode);
                return makeLabelText(this.target);
            }

            public string jumpAddress()
            {
                this.target = _jumpAddress(this.address, this.opcode);
                return makeLabelText(this.target);
            }
        ...
        static Func<Instruction, string>[] simpleTable =
        {
            ...
            (i) => { return "ADDI      " + i.rt_d() + " = " + i.rs() + " + " + i.immwd(); },
            (i) => { return "ADDIU     " + i.rt_d() + " = " + i.rs() + " + " + i.immwd(); },
