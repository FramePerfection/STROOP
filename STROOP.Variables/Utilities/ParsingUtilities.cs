...
namespace STROOP.Variables.Utilities
{
    public static class ParsingUtilities
    {
        ...
        {
            return (byte)GetIntegerInRangeWrapped(value, 1.0 + byte.MaxValue - byte.MinValue, false);
        }

        public static sbyte ParseSByteRoundingWrapping(double value)
        {
            return (sbyte)GetIntegerInRangeWrapped(value, 1.0 + sbyte.MaxValue - sbyte.MinValue, true);
        }

        public static short ParseShortRoundingWrapping(double value)
        {
            return (short)GetIntegerInRangeWrapped(value, 1.0 + short.MaxValue - short.MinValue, true);
        }

        public static ushort ParseUShortRoundingWrapping(double value)
        {
            return (ushort)GetIntegerInRangeWrapped(value, 1.0 + ushort.MaxValue - ushort.MinValue, false);
        }

        public static int ParseIntRoundingWrapping(double value)
        {
            return (int)GetIntegerInRangeWrapped(value, 1.0 + int.MaxValue - int.MinValue, true);
        }

        public static uint ParseUIntRoundingWrapping(double value)
        {
            return (uint)GetIntegerInRangeWrapped(value, 1.0 + uint.MaxValue - uint.MinValue, false);
        }
        ...
        {
            return (byte)GetIntegerInRangeCapped(value, 1.0 + byte.MaxValue - byte.MinValue, false);
        }

        public static sbyte ParseSByteRoundingCapping(double value)
        {
            return (sbyte)GetIntegerInRangeCapped(value, 1.0 + sbyte.MaxValue - sbyte.MinValue, true);
        }

        public static short ParseShortRoundingCapping(double value)
        {
            return (short)GetIntegerInRangeCapped(value, 1.0 + short.MaxValue - short.MinValue, true);
        }

        public static ushort ParseUShortRoundingCapping(double value)
        {
            return (ushort)GetIntegerInRangeCapped(value, 1.0 + ushort.MaxValue - ushort.MinValue, false);
        }

        public static int ParseIntRoundingCapping(double value)
        {
            return (int)GetIntegerInRangeCapped(value, 1.0 + int.MaxValue - int.MinValue, true);
        }

        public static uint ParseUIntRoundingCapping(double value)
        {
            return (uint)GetIntegerInRangeCapped(value, 1.0 + uint.MaxValue - uint.MinValue, false);
        }
        ...
        static double GetIntegerInRangeCapped(double value, double range, bool signed)
        {
            value = Math.Round(value, MidpointRounding.AwayFromZero);
            double min = signed ? -1 * range / 2 : 0;
            double exclusiveMax = signed ? range / 2 : range;
            double inclusiveMax = exclusiveMax - 1;
            return Math.Min(Math.Max(value, min), inclusiveMax);
        }
        ...
        static double MaybeNegativeModulus(double value, double modulus)
        {
            value %= modulus;
        ...
        static double NonNegativeModulus(double value, double modulus)
        {
            value %= modulus;
            if (value < 0) value += modulus;
            return value;
